...
def classify_format_1(row):
    ...
    # OVERRIDE: If balance_match succeeded, always return NO_ISSUES
    if balance_match == 'Success':
        return 'NO_ISSUES', 'Balance match succeeded. This overrides all other metadata checks'

    # Combination 6: Qt 4.8.7 with modification - FATAL (check before combination 1)
    if (meta_title == 'PDF Template' and
        meta_author == 'N/A' and
        meta_producer == 'Qt 4.8.7' and
        meta_created_at is not None and
        meta_modified_at is not None):
        return 'FATAL', 'PDF Template with Qt 4.8.7 but meta_modified_at is not NULL, indicating post-creation modification. Status: FLAGGED'
